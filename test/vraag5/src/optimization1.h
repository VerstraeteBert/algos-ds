...
bool BinaryTree::build(std::vector<std::string>& questions, std::list<std::string> &animals, std::vector<std::map<std::string, bool>> &answers, int level) {
    ...
    // OPTIMIZATION 1
    //  The question is redundant if either of the lists are empty
    //      -> rerun for current node with non empty list
    //
    if (animalsPos.empty()) {
        return this->build(questions, animals, answers, level + 1);
    } else if (animals.empty()) {
        return this->build(questions, animalsPos, answers, level + 1);
    }
    // if questions isn't redundant, add the question as the current node
